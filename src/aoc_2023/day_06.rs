...
pub fn find_multisolution_product(record_sheet: &Vec<&str>) -> u64 {
    ...
    let times_and_distances = record_sheet.iter()
        .map(|s| {
    ...
    for i in 0..times_and_distances[0].len() {
        let min = find_min_valid_time(times_and_distances[i][0], times_and_distances[i][1]);
        let max = find_max_valid_time(times_and_distances[i][0], times_and_distances[i][1]);
...
pub fn find_large_input_solutions(record_sheet: &Vec<&str>) -> u64 {
    let re = Regex::new("([0-9]{1,})").unwrap();
    let time_distance = record_sheet.iter()
        .map(|s|  
            re.find_iter(s)
