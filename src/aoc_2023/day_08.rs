...
pub fn find_steps_to_zzz(directions: &Vec<&str>) -> usize {
    ...
    while current != "ZZZ" {
        ...
        match lr_directions[steps % lr_directions.len()] {
            ...
            _ => panic!("Invalid step passed: '{}'", lr_directions[steps % lr_directions.len()])
        }
        steps += 1;
    }
...
pub fn ghost_traverse_to_exit_steps(directions: &Vec<&str>) -> usize {
    ...
    let out = positions.iter()
        .map(|pos| {
            let mut all_positions: Vec<String> = Vec::new();
            let mut current = pos.clone();

            let mut step = 0;
            while !all_positions.contains(&format!("{}_{}", current, lr_directions[step % lr_directions.len()])) {
                // cache that it happened
                all_positions.push(format!("{}_{}", current, lr_directions[step % lr_directions.len()]));
...
fn parse_map(directions: &Vec<&str>) -> HashMap<String, (String, String)> {
    ...
    for i in 2 .. directions.len() {
        let matches = re.find_iter(directions[i])
