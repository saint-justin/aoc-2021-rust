use std::{cmp::Ordering, collections::{HashSet, HashMap}, hash};
...
pub fn calculate_north_load_after_1b_cycles(initial_map: &Vec<&str>) -> u32 {
    ...
    for i in 0 .. 1_000_000_000 {
        ...
        if hashmap.contains_key(&shifted_map) {
            ...
            shifted_map = hashmap.clone().iter().find(|(_k, v)| v == &&out_index).unwrap().0.to_owned();
...
fn shift_rocks(initial_map: &Vec<Vec<String>>, direction: (isize, isize)) -> Vec<Vec<String>> {
    let mut map: Vec<Vec<String>> = initial_map.clone()
        .iter()
        .map(|row| row
            .iter()
...
fn shift_rock(initial_rock_pos: (isize, isize), map: &Vec<Vec<String>>, direction: (isize, isize)) -> (isize, isize) {
    match direction {
        NORTH => if initial_rock_pos.0 == 0                           { return initial_rock_pos },
        SOUTH => if initial_rock_pos.0 == (map.len() - 1) as isize    { return initial_rock_pos },
        WEST =>  if initial_rock_pos.1 == 0                           { return initial_rock_pos },
        EAST =>  if initial_rock_pos.1 == (map[0].len() - 1) as isize { return initial_rock_pos },
        _ => panic!("Invalid direction passed"),
    }

    let mut pos = initial_rock_pos.clone();
    while next_pos_in_bounds(pos, map, direction) {
        let to_check = ((pos.0 + direction.0) as usize, (pos.1 + direction.1) as usize);
...
fn next_pos_in_bounds(pos: (isize, isize), map: &Vec<Vec<String>>, direction: (isize, isize)) -> bool {
    match direction {
        NORTH => pos.0 > 0,
...
fn compare_rocks(r1: &(isize, isize), r2: &(isize, isize), direction: (isize, isize)) -> Ordering {
        match direction {
        NORTH => {
            if r1.0 > r2.0  { return Ordering::Greater };
            if r1.0 < r2.0  { return Ordering::Less };
            Ordering::Equal
        },
        SOUTH => {
            if r1.0 < r2.0  { return Ordering::Greater };
        ...
        WEST  => {
            if r1.1 > r2.1  { return Ordering::Greater };
            if r1.1 < r2.1  { return Ordering::Less };
            Ordering::Equal
        },
        EAST  => {
            if r1.1 < r2.1  { return Ordering::Greater };
